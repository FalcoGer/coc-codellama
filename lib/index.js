...
var CodeLlamaCompletionProvider = class {
  constructor(host, port, model, contextAbove, contextBelow) {
    this.host = host;
    this.port = port;
    this.model = model;
  ...
  async getCompletionItems(document, position) {
    ...
    const infillPrompt = "<PRE>" + document.getText(prefix) + " <SUF> " + document.getText(suffix) + " <MID>";
    let items = [];
    try {
...
async function activate(context) {
  import_coc.window.showInformationMessage("coc-codellama loaded!");
  ...
  let model = configuration.get("model", "codellama:7b-code");
  ...
  let source = new CodeLlamaCompletionProvider(host, port, model, contextAbove, contextBelow);
